  ...
  handleIngredientChange: function(changes) {
    ...
    render: function() {
      ...
      return (
        ...
        <h3>Ingredients</h3>
        {ingredientFields}
        <div>Choose your ingredients below:</div>
        {ingredientLinks}
        </div>
        <h4>Total: ${ingredientsTotal}</h4>
        </div>
        );
    }
});

var IngredientLink = React.createClass({
  handleClick: function(event) {
    ...
    render: function() {
      return(
        <a onClick={this.handleClick}>{this.props.ingredient.name + " "}</a>
        );
    }
});
...
var IngredientInputs = React.createClass({
  ...
  render: function() {
    ...
    return (
      <div>
      <input defaultValue={this.props.ingredient.quantity} type="number" className="quantity" onChange={this.handleChange} />{middleText}
      <input defaultValue={this.props.ingredient.cost} type="number" className="cost" onChange={this.handleChange} />{endText}
      </div>
    );
  }
});
