...
var CalculationBox = React.createClass({displayName: 'CalculationBox',
  render: function() {
    ...
    var grandTotal = Number(ingredientsTotal) + Number(suppliesTotal) + Number(timeTotal) + Number(overheadTotal) + Number(deliveryTotal);
    if (grandTotal > 0) {
      var panelFooter = React.DOM.span(null, "You should charge: ", React.DOM.strong({className: "pull-right"}, "$", grandTotal))
    } else {
      var panelFooter = React.DOM.span(null, "Do something on the left to get started!")
    }
    return (
