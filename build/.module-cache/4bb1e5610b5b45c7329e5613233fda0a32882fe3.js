...
var Calculator = React.createClass({displayName: 'Calculator',
  handleIngredientSelect: function(ingredient) {
    ...
    var pushIngredient = ingredient['key'] = runningKey
    runningKey = runningKey + 1;
    ingredients.push(pushIngredient);
    ...
    render: function() {
      ...
      var ingredientFields = this.props.ingredients.map(function(ingredient) {
        return (
...
var IngredientInputs = React.createClass({displayName: 'IngredientInputs',
  ...
  render: function() {
    ...
    return (
      React.DOM.div(null, 
      React.DOM.input({value: this.props.ingredient.quantity, type: "number", className: "quantity", onChange: this.handleChange}), middleText, 
      React.DOM.input({value: this.props.ingredient.cost, type: "number", className: "price", onChange: this.handleChange}), endText
      )
    );
  }
});
