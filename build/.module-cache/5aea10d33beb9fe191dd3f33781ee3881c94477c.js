  ...
  getInitialState: function() {
    ...
    render: function() {
      ...
      return (
          ...
          CalculationBox({ingredients: this.state.ingredients})
          )
          )
          );
    }
...
var CalculationBox = React.createClass({displayName: 'CalculationBox',
  render: function() {
    ...
    var suppliesSummary = this.props.supplys.map(function(supply) {
      return (
        React.DOM.p(null, supply.quantity, " ", supply.unit, " of ", supply.name, ": ", (supply.cost*supply.quantity).toFixed(2))
        )
    });
    return (
      React.DOM.div(null, ingredientsSummary)
      );
  }
});
