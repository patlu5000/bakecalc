...
var Calculator = React.createClass({displayName: 'Calculator',
    render: function() {
      return (
          ...
          TimeBox(null), 
          OverheadBox(null), 
          DeliveryBox({onUserInput: handleDeliveryInput})
          )
          );
    }
});
...
var IngredientInputs = React.createClass({displayName: 'IngredientInputs',
  handleChange: function(event) {
    ...
    render: function() {
      if ( _.contains(_.pluck(defaultIngredients, "key"), this.props.ingredient.key)) {
...
var TimeBox = React.createClass({displayName: 'TimeBox',
  handleChange: function(event) {
    ...
    render: function() {
      var timeTotal = (this.state.wage * this.state.hours).toFixed(2);
      return (
        React.DOM.div(null, 
        React.DOM.h3(null, "Time"), 
        React.DOM.input({type: "number", className: "hours", onChange: this.handleChange}), 
        "hours at",  
        React.DOM.input({type: "number", className: "wage", onChange: this.handleChange}), 
        "an hour", 
        React.DOM.h4(null, "Total: $", timeTotal)
        )
        );
    }
});
  ...
  handleChange: function(event) {
    ...
    render: function() {
      ...
      return(
        ...
        React.DOM.input({type: "number", className: "overhead", onChange: this.handleChange}), 
        React.DOM.h4(null, "Total: $", overheadTotal)
        )
        );
    }
});
  ...
  handleChange: function(event) {
    ...
    render: function() {
      ...
      return (
        ...
        React.DOM.input({type: "number", className: "miles", onChange: this.handleChange}), 
        "hours at",  
        React.DOM.input({type: "number", className: "rate", onChange: this.handleChange}), 
        "an hour", 
        React.DOM.h4(null, "Total: $", deliveryTotal)
        )
        );
    }
});
