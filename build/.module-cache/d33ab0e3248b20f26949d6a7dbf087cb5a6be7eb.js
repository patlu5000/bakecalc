...
var CalculationBox = React.createClass({displayName: 'CalculationBox',
  render: function() {
    ...
    return (
      ...
      CalculationItem({itemName: "Ingredients", itemTotal: suppliesTotal}), 
      CalculationItem({itemName: "Ingredients", itemTotal: timeTotal}), 
      CalculationItem({itemName: "Ingredients", itemTotal: overheadTotal}), 
      CalculationItem({itemName: "Ingredients", itemTotal: deliveryTotal})
      )
      )
      );
  }
...
var CalculationItem = React.createClass({displayName: 'CalculationItem',
  render: function() {
    if (typeof this.props.itemTotal != 'undefined' && this.props.itemTotal > 0) {
      return (React.DOM.li({className: "list-group-item"}, this.props.itemTotal));
    } else {
      return null;
    }
  }
