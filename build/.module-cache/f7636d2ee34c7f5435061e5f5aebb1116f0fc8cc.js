  ...
  handleSupplySelect: function(supply) {
    ...
    render: function() {
      ...
      return (
          ...
          supplyLinks, 
          SupplyLink({onSupplySelect: this.handleSupplySelect, supply: { quantity: 1, name: "", cost: null}})
          )
          );
    }
});
...
var SupplyLink = React.createClass({displayName: 'SupplyLink',
  handleClick: function(event) {
    ...
    render: function() {
      var displayName = this.props.supply.name || "something else"
      return(
        React.DOM.a({onClick: this.handleClick}, displayName)
        );
    }
  ...
  handleChange: function(event) {
    ...
    render: function() {
      if (_.contains(_.pluck(PRESET_SUPPLIES, "key"), this.props.supply.key)) {
        ...
        var hiddenInput = React.DOM.input({hidden: true, defaultValue: this.props.supply.name, type: "text", className: "name", onChange: this.handleChange});
      } else {
        var firstInput = React.DOM.input({defaultValue: this.props.supply.name, type: "text", className: "name", onChange: this.handleChange});
        var middleText = " at ";
        var hiddenInput = React.DOM.input({hidden: true, defaultValue: this.props.supply.quantity, type: "number", className: "quantity", onChange: this.handleChange});
      }
      return (
        React.DOM.div(null, 
        firstInput, 
        middleText, 
        React.DOM.input({defaultValue: this.props.supply.cost, type: "number", className: "cost", onChange: this.handleChange}), 
        hiddenInput
        )
        );
    }
