  ...
  handleChange: function(event) {
    ...
    render: function() {
      if (_.contains(_.pluck(PRESET_SUPPLIES, "key"), this.props.supply.key)) {
        ...
        var hiddenInput = React.DOM.input({defaultValue: this.props.supply.name, type: "text", className: "name", onChange: this.handleChange});
      } else {
        var firstInput = React.DOM.input({defaultValue: this.props.supply.name, type: "text", className: "name", onChange: this.handleChange});
        var middleText = " at ";
        var hiddenInput = React.DOM.input({defaultValue: this.props.supply.quantity, type: "number", className: "quantity", onChange: this.handleChange});
      }
      return (
        React.DOM.div(null, 
        firstInput, 
