...
var IngredientBox = React.createClass({displayName: 'IngredientBox',
  getInitialState: function() {
    ...
    handleIngredientSelect: function(ingredientName) {
    },

    render: function() {
      var ingredientLinks = _.difference(this.props.ingredients, this.state.ingredients).map(function(ingredient) {
        return (
          IngredientLink({onCommentSubmit: this.handleIngredientSelect, name: ingredient.name})
          );
      });
      return (
...
var IngredientLink = React.createClass({displayName: 'IngredientLink',
  handleClick: function(event) {
    ...
    render: function() {
      return(
        React.DOM.a({onClick: this.handleClick}, this.props.name + " ")
        );
    }
});
