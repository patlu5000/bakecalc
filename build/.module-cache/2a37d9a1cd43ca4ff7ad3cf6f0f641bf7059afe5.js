...
var Calculator = React.createClass({displayName: 'Calculator',
  handleIngredientSelect: function(ingredient) {
    ...
    render: function() {
      ...
      var ingredientsTotal = _.reduce(_.map(this.state.activeIngredients, function(ing){ return ing['cost'] * ing['quantity'] }), function(memo, num){ return memo + num; }, 0);
      return (
...
var IngredientInputs = React.createClass({displayName: 'IngredientInputs',
  ...
  render: function() {
    ...
    return (
      React.DOM.div(null, 
      React.DOM.input({value: this.props.ingredient.quantity, type: "number", className: "quantity"}), middleText, 
      React.DOM.input({value: this.props.ingredient.cost, type: "number", className: "price"}), endText
      )
    );
  }
});
